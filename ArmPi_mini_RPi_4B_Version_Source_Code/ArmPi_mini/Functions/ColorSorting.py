    ...
    while True:
        if __isRunning:        
            if detect_color != 'None' and start_pick_up: 
                #Move to the target position and the height is 6cm. Determine whether the specific position can be reached through the returned result.
                #If the running time is not given, automatically calculate and return result.
                ...
                if not __isRunning: # detect whether to stop game
                    continue
                AK.setPitchRangeMoving((coordinate['capture']), -90,-90, 0, 1000) # Robot arm moves to the pickig position
                ...
                if not __isRunning:
                    continue
                # =According to the different color, robot arm will turn to the different position
                if detect_color == 'red': 
                ...
                if not __isRunning:
                    continue
                result = AK.setPitchRangeMoving((coordinate[detect_color][0], coordinate[detect_color][1], 8), -90, -90, 0) # Robot arm moves above the gripping position
                if result == False:
                    unreachable = True
                else:
                    unreachable = False
                    time.sleep(result[2]/1000) #If reach to the specific position, the running time can be obtained.
                if not __isRunning:
                    continue
                AK.setPitchRangeMoving((coordinate[detect_color]), -90, -90, 0, 500) # robot arm moves to the picking position
                time.sleep(0.5)
                if not __isRunning:
                    continue
                ...
                if not __isRunning:
                    continue
                AK.setPitchRangeMoving((coordinate[detect_color][0], coordinate[detect_color][1], 8), -90, -90, 0, 800) # the robot arm raises up
                time.sleep(0.8)
                if not __isRunning:
                    continue
                # Robotic arm reset in stages
                Board.setPWMServosPulse([1200, 4, 1,1500, 3,515, 4,2170, 5,945])
                time.sleep(1.2)
                if detect_color == 'red':
