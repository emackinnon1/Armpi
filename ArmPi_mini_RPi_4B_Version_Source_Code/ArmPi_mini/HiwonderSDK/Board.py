...
def setBusServoID(oldid, newid):
    """
    configure servo ID and ID defaults to 1
    :param oldid: the original ID, defaults to 1
    :param newid: new ID
    """
...
def getBusServoID(id=None):
    """
    read servo ID  
    :param id: defaults to empty
    :return: return servo ID
    """
    
    while True:
        if id is None:  # Only a single servo can be connected to this servo port
...
def getBusServoAngleLimit(id):
    ...
    :param id:
    :return: return tuple 0： low-bit  1： hig-bit
    '''
...
def setBusServoMaxTemp(id, m_temp):
    '''
    set the highest alarm temperature
    :param id:
    :param m_temp:
    :return:
    '''
...
def getBusServoTempLimit(id):
    '''
    read the range of servo alarm temperature
    :param id:
    :return:
    '''
