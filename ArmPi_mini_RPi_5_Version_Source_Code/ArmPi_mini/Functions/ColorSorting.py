        ...
        if __isRunning:        
            if detect_color != 'None' and start_pick_up: 
                ...
                if not __isRunning: # 检测是否停止玩法
                    continue
                AK.setPitchRangeMoving((coordinate['capture']), -90,-90, 0, 1000) # 机械臂运行到夹取位置
                time.sleep(1) # 延时1秒
                if not __isRunning:
                    continue
                ...
                if not __isRunning:
                    continue
                result = AK.setPitchRangeMoving((coordinate[detect_color][0], coordinate[detect_color][1], 8), -90, -90, 0) # 机械臂运行到夹取位置上方
                if result == False:
                    unreachable = True
                else:
                    unreachable = False
                ...
                if not __isRunning:
                    continue
                AK.setPitchRangeMoving((coordinate[detect_color]), -90, -90, 0, 500) # 机械臂运行到夹取位置
                time.sleep(0.5)
                if not __isRunning:
                    continue
                ...
                if not __isRunning:
                    continue
                AK.setPitchRangeMoving((coordinate[detect_color][0], coordinate[detect_color][1], 8), -90, -90, 0, 800) # 抬起机械臂
                time.sleep(0.8)
                if not __isRunning:
                    continue
                # 机械臂分阶段复位
